...
const INJECTED_STYLES = `
  ...
  .text-silver-matte {
      background: linear-gradient(180deg, var(--color-foreground, #fff) 0%, color-mix(in srgb, var(--color-foreground, #fff) 40%, transparent) 100%);
      -webkit-background-clip: text;
      -webkit-text-fill-color: transparent;
      background-clip: text;
      transform: translateZ(0); /* Hardware acceleration to prevent WebKit clipping bug */
      filter: 
          drop-shadow(0px 10px 20px color-mix(in srgb, var(--color-foreground, #fff) 15%, transparent)) 
          drop-shadow(0px 2px 4px color-mix(in srgb, var(--color-foreground, #fff) 10%, transparent));
  }
