...
from fastapi import FastAPI, WebSocket, Request, Depends, status
...
def create_app() -> FastAPI:
    ...
    # 1. CORS Configuration (Narrowly scoped for local frontend development)
    ...
    # 3. Health & Introspection Endpoint
    @app.get(
        "/api/agent/health",
    ...
    # 4. Tool Registry Introspection Endpoint
    @app.get(
        "/api/agent/tools",
    ...
    # 7. Standard Health Endpoint Alias
    @app.get("/api/health", response_model=HealthResponse, tags=["System"])
    def standard_health():
        return health_check()

    # 8. Presets Discovery Endpoint
    @app.get("/api/presets", response_model=PresetsResponse, tags=["Engineering"])
    def get_presets():
    ...
    # 9. Direct Simulation Endpoint (Synchronized FNO + Physics Reference)
    @app.post("/api/simulate", response_model=SimulateResponse, tags=["Engineering"])
    ...
    # 10. Parametric Sweep Endpoint
    @app.post("/api/sweep", response_model=SweepApiResponse, tags=["Engineering"])
    ...
    # 11. Engineering Report Endpoint
    @app.post("/api/report", response_model=ReportApiResponse, tags=["Engineering"])
    ...
    # 12. Mount Production Static Frontend Bundle (SPA)
